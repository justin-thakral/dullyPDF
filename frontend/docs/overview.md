...
- When post-submit signing is enabled for a template link, the owner must map a signer name question and signer email question. The backend creates the signing request from the stored response snapshot, materializes the immutable filled PDF server-side, then sends the signing invite to the mapped signer email instead of returning a live signer link to the current browser. Once the signer opens that emailed `/sign/:token` route, the first step is email verification for that session, just like the owner-created email-signing flow.
- If that email delivery fails transiently, the respondent success screen now exposes a resend action that reuses the stored Fill By Link response instead of asking them to refill the form.
