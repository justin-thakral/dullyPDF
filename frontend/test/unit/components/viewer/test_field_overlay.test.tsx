  ...
  it('updates geometry for each resize handle and enforces minimum size', () => {
    const testCases: Array<{
    ...
    }> = [
      {
        ...
        moveTo: { x: 120, y: 20 },
        expected: { x: 34, y: 10, width: 6, height: 20 },
      },
      {
      ...
      },
      {
        ...
        moveTo: { x: 20, y: 100 },
        expected: { x: 10, y: 24, width: 30, height: 6 },
      },
      {
        handleClass: '.field-handle--bottom',
        moveTo: { x: 20, y: 80 },
        expected: { x: 10, y: 0, width: 30, height: 80 },
      },
      {
        handleClass: '.field-handle--br',
        moveTo: { x: 80, y: 30 },
        expected: { x: 10, y: 10, width: 90, height: 60 },
      },
    ];
    ...
    for (const testCase of testCases) {
      ...
      pointerMove(testCase.moveTo.x, testCase.moveTo.y);
      pointerUp();
