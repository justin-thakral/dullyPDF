/**
 * Modal for configuring and testing a database connection.
 */
import React, { useState } from 'react';
import './ConnectDB.css';
import { DB } from './db';
import type { DbConnectionConfig, DbType } from './db';
...
/**
 * Collect DB credentials and create a backend connection record.
 */
const ConnectDB: React.FC<Props> = ({ open, onClose, onConnected }) => {
  ...
  const [type, setType] = useState<DbType>(DEFAULTS.type);
  ...
  const [ssl, setSsl] = useState(DEFAULTS.ssl);
  ...
  /**
   * Validate input fields and call the backend connection test.
   */
