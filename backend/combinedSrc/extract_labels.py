...
def _extract_labels_from_page(
    ...
    *,
    ...
    if render is not None and len(words or []) < 8:
        if render.get("image") is not None:
            logger.warning(
                "Page %s has %s extractable words; running OCR fallback for labels.",
                page_idx,
                len(words or []),
            )
            page_box = PageBox(
            ...
            try:
                ...
                ocr_words = ocr_words_to_labels(
                    render["image"], page_box=page_box, min_confidence=35
                )
                if len(ocr_words) >= max(12, int(len(words or []) * 2.0)):
                    words = ocr_words
            except Exception as exc:
                logger.warning("OCR failed on page %s: %s", page_idx, exc)

    words = _filter_words(words or [])
    labels = _group_words_into_lines(words)
    logger.debug(
