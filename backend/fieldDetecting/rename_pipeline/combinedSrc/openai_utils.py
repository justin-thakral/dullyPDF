"""
OpenAI helper utilities for the rename pipeline.

These wrappers normalize SDK quirks (like models that reject `temperature`) and
provide a safe way to extract text across response payload variations.
"""
...
def responses_create_with_temperature_fallback(
    ...
    **kwargs: Any,
    ...
    """
    # Use a single params dict so we can retry with a clean delta if temperature is unsupported.
    params: Dict[str, Any] = {
        "model": model,
        "input": input,
        "text": text,
