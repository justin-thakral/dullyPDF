"""
Standardized output directory layout for rename pipeline artifacts.
"""
...
class OutputLayout:
    """
    Standardized output layout for rename pipeline runs.
...
def temp_prefix_from_pdf(pdf_path: Path, *, fallback: Optional[str] = None) -> str:
    """
    Build the standardized temp prefix used for rename pipeline artifacts.

    Format: temp + first 5 + last 5 characters of the PDF stem (lowercased)
    plus a short path hash to avoid collisions across directories.
    """
    stem = (pdf_path.stem or fallback or "file").lower()
    head = stem[:5]
    tail = stem[-5:] if len(stem) > 5 else stem
    try:
        path_token = pdf_path.resolve().relative_to(Path.cwd().resolve()).as_posix()
    except ValueError:
        path_token = str(pdf_path.resolve())
    digest = hashlib.md5(path_token.encode("utf-8")).hexdigest()[:6]
    return f"temp{head}{tail}_{digest}"
