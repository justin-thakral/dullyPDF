...
_OCR_MIN_WORDS = int(os.getenv("SANDBOX_OCR_MIN_WORDS", "8"))
_OCR_MAX_WORDS = int(os.getenv("SANDBOX_OCR_MAX_WORDS", "120"))
_OCR_MIN_ALPHA_RATIO = float(os.getenv("SANDBOX_OCR_MIN_ALPHA_RATIO", "0.35"))
_OCR_MIN_GAIN = float(os.getenv("SANDBOX_OCR_MIN_GAIN", "2.0"))
_OCR_MIN_CONFIDENCE = int(os.getenv("SANDBOX_OCR_MIN_CONFIDENCE", "35"))
...
def _word_has_alpha(text: str) -> bool:
...
def _extract_labels_from_page(
    ...
    *,
    ...
    Data structures:
    - Input: pdfplumber Page (vector text extraction) and optional rendered image for OCR.
    - Output: list of {text, bbox} label dicts in PDF point coordinates (originTop).

    Runtime: O(w log w) for sorting/grouping words; OCR fallback can add extra cost.
    """
    ...
    alpha_count = sum(1 for w in filtered_words if _word_has_alpha(w.get("text")))
    alpha_ratio = float(alpha_count) / float(max(1, filtered_count))

    should_ocr = (
        render is not None
        and render.get("image") is not None
        and (
            filtered_count < _OCR_MIN_WORDS
            or (filtered_count <= _OCR_MAX_WORDS and alpha_ratio < _OCR_MIN_ALPHA_RATIO)
        )
    )
    if should_ocr:
        logger.warning(
            "Page %s has %s usable words (alpha_ratio=%.2f); running OCR fallback for labels.",
            page_idx,
            filtered_count,
            alpha_ratio,
        )
        page_box = PageBox(
            page_width=float(render.get("width_points") or page.width),
            page_height=float(render.get("height_points") or page.height),
            rotation=int(render.get("rotation") or page.rotation or 0),
        )
        try:
            from .ocr_labels import ocr_words_to_labels

            ocr_words = ocr_words_to_labels(
                render["image"], page_box=page_box, min_confidence=_OCR_MIN_CONFIDENCE
            )
            min_required = max(_OCR_MIN_WORDS, int(filtered_count * _OCR_MIN_GAIN))
            if len(ocr_words) >= min_required:
                filtered_words = _filter_words(ocr_words)
