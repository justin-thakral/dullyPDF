...
TEXT_LAYER_MIN_ALPHA_RATIO = float(
    os.getenv("SANDBOX_TEXT_LAYER_MIN_ALPHA_RATIO", "0.30")
)
...
class TextLayerStats:
    ...
    def alpha_ratio(self) -> float:
        if self.total_words <= 0:
            return 0.0
        return float(self.total_alpha_words) / float(self.total_words)
...
def summarize_text_layer(pdf_bytes: bytes) -> TextLayerStats:
    ...
    with pdfplumber.open(io.BytesIO(pdf_bytes)) as pdf:
        for idx, page in enumerate(pdf.pages):
            ...
            count = len(words)
            alpha_count = sum(
                1 for w in words if any(ch.isalpha() for ch in (w.get("text") or ""))
            )
            page_idx = idx + 1
            words_by_page[page_idx] = count
            total_words += count
            total_alpha_words += alpha_count
            if count > 0:
                pages_with_text += 1
    ...
    logger.debug(
        "Text-layer stats: pages=%s total_words=%s avg_words=%.2f alpha_ratio=%.2f pages_with_text=%s",
        stats.total_pages,
        stats.total_words,
...
def is_reliable_text_layer(stats: TextLayerStats) -> bool:
    """
    Return True when the text layer looks usable for glyph overlap filters.
    """
    if stats.total_words <= 0:
        return False
    return stats.alpha_ratio >= TEXT_LAYER_MIN_ALPHA_RATIO
