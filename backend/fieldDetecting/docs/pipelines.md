...
- **Native pipeline** (`backend/fieldDetecting/sandbox/nativeSrc/`): For PDFs with a usable text layer.
- **Scanned pipeline** (`backend/fieldDetecting/sandbox/scannedSrc/`): For scanned/image-first PDFs where the text layer is missing or unreliable.
...
The router lives in `backend/fieldDetecting/sandbox/combinedSrc/pipeline_router.py`.
...
1) **Rendered pages**: `backend/fieldDetecting/sandbox/combinedSrc/render_pdf.py`
...
2) **Geometry candidates**: `backend/fieldDetecting/sandbox/combinedSrc/detect_geometry.py`
...
3) **Label candidates**: `backend/fieldDetecting/sandbox/combinedSrc/extract_labels.py`
   - Attempts pdfplumber text extraction first.
   - Falls back to OCR when a page has 0 extractable words (`backend/fieldDetecting/sandbox/combinedSrc/ocr_labels.py`).

4) **Candidate assembly**: `backend/fieldDetecting/sandbox/combinedSrc/build_candidates.py`
...
5) **Calibration**: `backend/fieldDetecting/sandbox/combinedSrc/calibration.py`
   - Computes median label height per page for more stable rect sizing heuristics.

## Native resolver (text-layer)

Location: `backend/fieldDetecting/sandbox/combinedSrc/heuristic_resolver.py` (invoked by `backend/fieldDetecting/sandbox/nativeSrc/pipeline.py`)
...
Location: `backend/fieldDetecting/sandbox/combinedSrc/heuristic_resolver.py` (invoked by `backend/fieldDetecting/sandbox/scannedSrc/pipeline.py`)
...
Location: `backend/fieldDetecting/sandbox/combinedSrc/rename_resolver.py`
