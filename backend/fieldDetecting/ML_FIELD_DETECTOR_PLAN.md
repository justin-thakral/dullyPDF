...
- `backend/fieldDetecting/sandbox/combinedSrc/render_pdf.py` renders at 500 DPI today.
...
Example structure:
- `backend/fieldDetecting/sandbox/ML/data/raw/images/...`
- `backend/fieldDetecting/sandbox/ML/data/raw/annotations/train.json`
- `backend/fieldDetecting/sandbox/ML/data/raw/annotations/val.json`

### 5.2 Generate tiled training set
Write a deterministic script (proposed filename):
- `backend/fieldDetecting/sandbox/ML/prepare_tiled_dataset.py`
...
4) Save:
   - `backend/fieldDetecting/sandbox/ML/data/tiles/images/<page>__x<tileX>_y<tileY>.png`
   - `backend/fieldDetecting/sandbox/ML/data/tiles/labels/<same>.txt` in YOLO format
...
Proposed module:
- `backend/fieldDetecting/sandbox/ML/ml_detector.py`
...
- `backend/fieldDetecting/sandbox/combinedSrc/coords.py` (`px_to_pts`, `PageBox`)
...
### 9.1 Golden PDFs
Pick a small set of “golden” PDFs (including `backend/fieldDetecting/pdfs/scanned/medical-history-intake-form.pdf`) and maintain:
...
- `backend/fieldDetecting/sandbox/ML/validate_ml_outputs.py`
...
1) run `sandbox/debug/test_rects.py` to produce overlays
2) scan overlays for:
   - missed underlines
   - table border noise
   - “O” false checkboxes
...
## 14) Recommended ML upgrades (layout + key-value reasoning)

The current plan focuses on **object detection**. The next gains come from **layout-aware reasoning**:
- separating headers/tables from inputs
- linking labels to fields in dense layouts
- handling scanned PDFs where OCR is brittle

### 14.1 Layout analysis model (DocLayNet / PubLayNet style)
Goal: detect layout blocks (title/header, table, text blocks) so the detector can suppress
underline/box candidates in non-input regions.

Steps:
1) Pick a layout dataset (DocLayNet or PubLayNet) and fine-tune a lightweight detector.
2) Export predicted layout regions as masks (table/header/body).
3) Add a post-filter that suppresses candidates inside header/table regions unless strong
   geometric evidence exists (e.g., a real checkbox cluster).
4) Keep this as a **soft prior** (e.g., reduce confidence), not a hard deletion at first.

### 14.2 Key-value relation model (LayoutLM / KVPFormer)
Goal: reduce label-to-field mismatches with a global, layout-aware linker.

Steps:
1) Create a training set where each field candidate is linked to the correct label.
2) Train a model to predict label->field edges using text + bbox features.
3) Use this model only when the heuristic resolver confidence is low.
4) Fall back to heuristics if the model is uncertain or out-of-distribution.

Minimal integration path:
- Convert candidates/labels into a token list with bbox coordinates.
- Score all label<->field pairs and re-rank the heuristic assignments.

### 14.3 OCR-free fallback (Donut-style)
Goal: handle scan-heavy PDFs where OCR misses labels entirely.

Steps:
1) Start with a pre-trained Donut model and fine-tune on form images with JSON outputs.
2) Use Donut to propose candidate fields + label text in a single pass.
3) Merge Donut outputs with the object detector (union + dedupe).
4) Gate usage to pages with weak OCR/text-layer stats to avoid regressions.

### 14.4 Validation milestones for ML upgrades
- Layout model: fewer false positives inside headers/tables without hurting recall.
- Key-value model: fewer label mismatches on dense forms (measure edge accuracy).
- OCR-free fallback: improved recall on scanned PDFs with near-zero text layer.
