...
def test_session_last_access_coerces_last_access_and_created_at() -> None:
    assert store._session_last_access({"last_access": "12.5"}) == 12.5
    assert store._session_last_access({"created_at": "7"}) == 7.0
    assert store._session_last_access({"last_access": 0, "created_at": 9}) == 9.0


def test_session_last_access_returns_zero_for_malformed_values() -> None:
    assert store._session_last_access({"last_access": object()}) == 0.0
    assert store._session_last_access({"created_at": "not-a-number"}) == 0.0


def test_prune_session_cache_removes_expired_entries(monkeypatch) -> None:
    monkeypatch.setattr(store, "_SESSION_TTL_SECONDS", 100)
    monkeypatch.setattr(store, "_SESSION_SWEEP_INTERVAL_SECONDS", 0)
    store._LAST_SESSION_SWEEP = 0.0
    store._API_SESSION_CACHE.update(
        OrderedDict(
            [
    ...
    store._prune_session_cache(now=200.0)

    assert list(store._API_SESSION_CACHE.keys()) == ["fresh"]
    assert store._LAST_SESSION_SWEEP == 200.0


def test_prune_session_cache_short_circuits_within_sweep_interval(monkeypatch) -> None:
    monkeypatch.setattr(store, "_SESSION_TTL_SECONDS", 100)
    monkeypatch.setattr(store, "_SESSION_SWEEP_INTERVAL_SECONDS", 30)
    store._LAST_SESSION_SWEEP = 190.0
    store._API_SESSION_CACHE["expired"] = {"last_access": 10.0}

    store._prune_session_cache(now=200.0)

    assert list(store._API_SESSION_CACHE.keys()) == ["expired"]
    assert store._LAST_SESSION_SWEEP == 190.0


def test_trim_session_cache_size_evicts_lru(monkeypatch) -> None:
    monkeypatch.setattr(store, "_SESSION_MAX_ENTRIES", 2)
    store._API_SESSION_CACHE.update(
        OrderedDict(
            [
                ("a", {"last_access": 1.0}),
    ...
    store._trim_session_cache_size()

    assert list(store._API_SESSION_CACHE.keys()) == ["b", "c"]


def test_trim_session_cache_size_noop_when_max_entries_zero(monkeypatch) -> None:
    monkeypatch.setattr(store, "_SESSION_MAX_ENTRIES", 0)
    store._API_SESSION_CACHE.update(OrderedDict([("a", {}), ("b", {})]))

    store._trim_session_cache_size()

    assert list(store._API_SESSION_CACHE.keys()) == ["a", "b"]
...
def test_store_l1_entry_sets_timestamps_and_updates_lru_order(monkeypatch) -> None:
    ...
    monkeypatch.setattr(store, "_SESSION_SWEEP_INTERVAL_SECONDS", 0)
    monkeypatch.setattr(store, "_SESSION_TTL_SECONDS", 1000)
    monkeypatch.setattr(store, "_SESSION_MAX_ENTRIES", 10)

    store._store_l1_entry("a", {"payload": "first"})
    store._store_l1_entry("b", {"payload": "second"})
    store._store_l1_entry("a", {"payload": "updated"})

    assert list(store._API_SESSION_CACHE.keys()) == ["b", "a"]
    assert store._API_SESSION_CACHE["a"]["created_at"] == 3.0
    assert store._API_SESSION_CACHE["a"]["last_access"] == 3.0
    assert store._API_SESSION_CACHE["a"]["payload"] == "updated"


def test_session_object_path_trims_inputs_and_builds_expected_path() -> None:
    assert store._session_object_path("  sess-1  ", " /fields.json ") == "sessions/sess-1/fields.json"


def test_session_object_path_rejects_missing_session_id() -> None:
    with pytest.raises(ValueError, match="Missing session_id"):
        store._session_object_path("   ", "fields.json")
...
def test_session_object_path_rejects_missing_suffix() -> None:
    with pytest.raises(ValueError, match="Missing session artifact suffix"):
        store._session_object_path("sess-1", " / ")
...
def test_expires_at_returns_utc_timestamp_when_ttl_positive(monkeypatch) -> None:
    monkeypatch.setattr(store, "_SESSION_TTL_SECONDS", 30)
    before = datetime.now(timezone.utc)

    value = store._expires_at()
...
def test_session_last_access_zero_last_access_no_created_at_falls_through() -> None:
    ...
    result = store._session_last_access({"last_access": 0})
    assert result == 0.0
...
def test_session_last_access_empty_dict_returns_zero() -> None:
    ...
    result = store._session_last_access({})
    assert result == 0.0
...
def test_prune_session_cache_early_returns_when_ttl_non_positive(monkeypatch) -> None:
    ...
    monkeypatch.setattr(store, "_SESSION_SWEEP_INTERVAL_SECONDS", 0)
    store._LAST_SESSION_SWEEP = 0.0
    store._API_SESSION_CACHE["old"] = {"last_access": 1.0}

    store._prune_session_cache(now=99999.0)

    # The entry must still be present because pruning was disabled.
    assert "old" in store._API_SESSION_CACHE
    # _LAST_SESSION_SWEEP must remain unchanged.
    assert store._LAST_SESSION_SWEEP == 0.0
...
def test_expires_at_returns_none_when_ttl_is_negative(monkeypatch) -> None:
    ...
    monkeypatch.setattr(store, "_SESSION_TTL_SECONDS", -5)
