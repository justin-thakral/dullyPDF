...
def _resolve_saved_form_fill_rules(template_metadata: Optional[Dict[str, Any]]) -> Dict[str, List[Dict[str, Any]]]:
    ...
    checkbox_hints = fill_rules.get("checkboxHints") if isinstance(fill_rules.get("checkboxHints"), list) else metadata.get("checkboxHints")
    ...
    return {
        ...
        "checkboxHints": _coerce_dict_list(checkbox_hints),
...
def build_template_fill_link_download_snapshot(
    *,
    ...
    return {
        ...
        "checkboxHints": fill_rules["checkboxHints"],
...
def _resolve_option_aliases(group_options: Dict[str, List[Dict[str, Any]]]) -> Dict[str, str]:
    ...
    for option_key, entries in group_options.items():
        ...
        for entry in entries:
            option_label = normalize_data_key(str(entry.get("optionLabel") or ""))
            if option_label:
...
def _apply_checkbox_rule_to_group(
    *,
    ...
    presence = _coerce_checkbox_presence(raw_value)
    if presence is None:
        return False
