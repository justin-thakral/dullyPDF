...
- Debug gate (required for non-production toggles):
  ...
  - Example (CLI): `python -m backend.debug.test_local --debug path/to/sample.pdf`
...
- Optional tuning:
  ...
  - `export SANDBOX_LOG_DIR=backend/logs` to redirect log file output.
...
### Local test script
```
python -m backend.debug.test_local path/to/sample.pdf
# Output defaults to backend/outputArtifacts/json/temp<first5><last5>_fields.json
```
...
Overlay/debug helper (writes to `backend/outputArtifacts/` by default, with `json/` + `overlays/` subfolders):
```
python -m backend.debug.test_rects backend/pdfs/native/consent/base_form.pdf
```

PDF sorter (classifies PDFs into native/scanned groups, default mode is `move`):
```
python -m backend.tools.sort_pdfs --pdfs-dir backend/pdfs
```
Options:
- `--forms-dir backend/forms/native` (or another forms root) to classify filled PDFs.
- `--forms-output-root backend/forms` to choose where classified forms land (default is `forms/`).
- `--mode copy` to copy files instead of moving.

ML corpus migration (moves ML PDFs into the new native/scanned layout):
```
python -m backend.tools.migrate_ml_pdfs
```
Outputs:
- `backend/pdfs/native`
- `backend/pdfs/scanned`
Note: ML migration only moves input PDFs; it does not emit JSON or overlays.

### Input PDFs
- Source PDFs live in `backend/pdfs/native/` and `backend/pdfs/scanned/` (subfolders: `hippa/`, `consent/`, `intake/`).
- `backend/pdfs/manifest.json` tracks native vs scanned classifications for sandbox runs.

### Sandbox layout
- `combinedSrc/` shared pipeline code used by both native + scanned flows.
- `nativeSrc/` text-layer pipeline logic.
- `scannedSrc/` scanned pipeline logic.
- `ML/` ML-specific scripts and datasets.
- `debug/` debug CLI helpers (Python-only; reference images live in `docs/images/`).
- `test/` pytest files.
- `tools/` one-off maintenance utilities (sorting, batch ops).
- `forms/` output PDFs split into `native/` and `scanned/`.
- `outputArtifacts/` contains `json/` and `overlays/` for pipeline/debug outputs.
- `tools/artifactManager.py` deletes temp-prefixed debug artifacts.
- `tools/migrate_analysis_artifacts.py` migrates legacy `analysis_*` runs into `json/` + `overlays/`.

### What it does
1) `combinedSrc/render_pdf.py` renders each page at 500 DPI with PyMuPDF and records pixel-to-point scaling.
2) `combinedSrc/detect_geometry.py` finds underlines, rectangles, and checkbox-like squares with OpenCV and converts them back to PDF points (origin top-left).
3) `combinedSrc/extract_labels.py` groups text lines with pdfplumber into label candidates.
4) `combinedSrc/build_candidates.py` merges geometry + labels into per-page candidate JSON.
5) `combinedSrc/rename_resolver.py` optionally calls OpenAI on full-page overlays to rename fields and filter low-confidence fields.
6) `main.py` wraps the pipeline behind `/detect-fields`; `debug/test_local.py` runs it from the CLI.
7) Routing is in `combinedSrc/pipeline_router.py`, with pipeline entrypoints in `nativeSrc/` and `scannedSrc/`.
...
- Sandbox logs are also written to `backend/logs/sandbox.log` by default.
