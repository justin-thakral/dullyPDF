...
The sandbox also tries to load keys from `backend/.env` if present (see `backend/combinedSrc/config.py`).
...
The main CLI lives at `backend/debug/test_local.py`:

```bash
python3 -m backend.debug.test_local path/to/sample.pdf
```
...
```bash
python3 -m backend.debug.test_local path/to/sample.pdf --pipeline auto
```

- Force native:
```bash
python3 -m backend.debug.test_local path/to/sample.pdf --pipeline native
```

- Force scanned:
```bash
python3 -m backend.debug.test_local path/to/sample.pdf --pipeline scanned
```
...
```bash
python3 -m backend.debug.test_rects backend/pdfs/native/consent/base_form.pdf
```

Outputs are written under `backend/outputArtifacts/` by default:
- `backend/outputArtifacts/json/*.json` (temp-prefixed, e.g. `temp<first5><last5>_fields.json`)
- `backend/outputArtifacts/overlays/*.png`
...
```bash
python3 -m backend.debug.test_local path/to/sample.pdf --openAI
```

Outputs:
- `backend/outputArtifacts/json/temp<first5><last5>_renames.json`
- `backend/outputArtifacts/json/temp<first5><last5>_fields_renamed.json`
- `backend/outputArtifacts/overlays/temp<first5><last5>_openai/page_<n>.png`
...
```bash
python3 -m backend.combinedSrc.form_filler input.pdf template.json --output output_form.pdf
```
