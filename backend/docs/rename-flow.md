...
Files:
- `backend/combinedSrc/field_overlay.py`: renders the overlay image and prints the field name above each field.
- `backend/combinedSrc/rename_resolver.py`: calls OpenAI and applies renames to fields.
...
Artifacts:
- Overlays: `backend/outputArtifacts/overlays/temp<first5><last5>_openai/page_<n>.png`
- Renames: `backend/outputArtifacts/json/temp<first5><last5>_renames.json`
- Fields with names applied: `backend/outputArtifacts/json/temp<first5><last5>_fields_renamed.json`
